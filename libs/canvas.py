...
class Canvas(QWidget):
        ...
        if self.drawing():
            ...
            if self.current:
                ...
                if self.draw_square:
                    ...
                    # print(f"self.line[1]  : {self.line[1] }")
                else:
                    self.line[1] = pos
    ...
    def mousePressEvent(self, ev):
        ...
        if ev.button() == Qt.LeftButton:
            if self.drawing():
                # print(f"pos in mousePressEvent : {pos}")
    ...
    def mouseReleaseEvent(self, ev):
        if ev.button() == Qt.RightButton:
            ...
            pos = self.transform_pos(ev.pos())
            if self.drawing():
                # print(f"pos in mouse release  : {pos}")
                self.handle_drawing(pos)
            else:
                # pan
    ...
    def croped_file_save(self, minx1, miny1, maxx2, maxy2):
        img = self.pixmap.toImage()

        # print(type(img))
        # img = .grab(bbox=(minx1, miny1, maxx2, maxy2))
        saved_img_name = f"{int(minx1)}_{int(miny1)}.jpg"
        saved_img_path = os.path.join(os.getcwd(), 'output')
        try:
        ...
        hight = maxy2 - miny1
        width = maxx2 - minx1
        rect = QRect(minx1, miny1, width, hight)
        print(f'rect ; {rect}')
        image = img.copy(rect)
        image.save(os.path.join(saved_img_path, saved_img_name))
    ...
    def handle_drawing(self, pos):
        if self.current and self.current.reach_max_points() is False:
            ...
            self.croped_file_save(min_x, min_y, max_x, max_y)
            self.finalise()
